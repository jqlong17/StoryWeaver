...
def log_api_interaction(call_type, prompt, response, timestamp=None):
    log_dir = "logs"
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, f"deepseek_api_{timestamp if timestamp else 'unknown'}.log")
...
# DeepSeek API统一调用
def call_deepseek_api(prompt, call_type="unknown", timestamp=None):
    headers = {
        "Authorization": f"Bearer {DEEPSEEK_API_KEY}",
        "Content-Type": "application/json"
    }
    ...
    log_api_interaction(call_type, prompt, content, timestamp=timestamp)
    return content
...
# UI生成
def get_deepseek_ui(keyword, scenes=None, timestamp=None):
    ...
    return call_deepseek_api(prompt, call_type="ui", timestamp=timestamp)
...
# 互动机制生成
def get_deepseek_interaction_mechanics(keyword, scene=None, main_story=None, timestamp=None):
    context = ''
    if main_story:
        context += f"主线剧情信息：{main_story}\n"
    if scene:
    ...
    return call_deepseek_api(prompt, call_type="interaction", timestamp=timestamp)

# 剧情分支/决策点生成
def get_deepseek_story_branches(keyword, scene=None, main_story=None, timestamp=None):
    context = ''
    if main_story:
        context += f"主线剧情信息：{main_story}\n"
    if scene:
    ...
    return call_deepseek_api(prompt, call_type="branch", timestamp=timestamp)

def parse_story_structure_and_generate_scene_details(story_structure, character_list, animation_list, appearance_list, ui_list, output_dir, keyword=None, main_story=None, timestamp=None):
    scenes_details = []
    appearance_map = {}
    if appearance_list:
    ...
    for chapter in story_structure.get('chapters', []):
        ...
        for scene in chapter.get('scenes', []):
            ...
            try:
                if keyword:
                    result = get_deepseek_interaction_mechanics(keyword, scene=scene_context, main_story=main_story, timestamp=timestamp)
                    result_extracted = extract_json(result)
                    if result_extracted is None:
                        print(f"[WARNING] 互动机制AI输出无法提取为有效JSON，chapter={chapter_id}, scene={scene_id}, title={scene_title}")
                        debug_file_path = os.path.join(output_dir, f"debug_interaction_{chapter_id}_{scene_id}.txt")
                        extract_json(result, debug_file_path=debug_file_path)
                        interactions_data = []
                    else:
                        parsed = json.loads(result_extracted)
            ...
            try:
                if keyword:
                    result = get_deepseek_story_branches(keyword, scene=scene_context, main_story=main_story, timestamp=timestamp)
                    result_extracted = extract_json(result)
                    if result_extracted is None:
                        print(f"[WARNING] 分支决策AI输出无法提取为有效JSON，chapter={chapter_id}, scene={scene_id}, title={scene_title}")
                        debug_file_path = os.path.join(output_dir, f"debug_branch_{chapter_id}_{scene_id}.txt")
                        extract_json(result, debug_file_path=debug_file_path)
                        branches_data = []
                    else:
                        parsed = json.loads(result_extracted)
...
def main():
    ...
    try:
        ...
        result = get_deepseek_ui(keyword, scenes=scenes_str, timestamp=timestamp)
        result = extract_json(result)
    ...
    try:
        ...
        if story_structure and character_list:
            parse_story_structure_and_generate_scene_details(
                story_structure, character_list, animation_list, appearance_list, ui_list, output_dir, keyword=keyword, main_story=json.dumps(main_story_obj, ensure_ascii=False) if main_story_obj else None, timestamp=timestamp
            )
        else:
